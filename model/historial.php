...
class Historial
{
    ...
    public function Listar()
    {
        try {
            $result = array();
            $stm = $this->pdo->prepare("SELECT hist.idHistorial, hist.nombreMedicamenoHistorial, 
            hist.docificacionHistorial, hist.fechaInicioHistorial, hist.fechaFinalHistorial, hist.propositoHisotrial
            as proposito, concat(d.apellidosDueno,'-',d.ciDueno) as dueno, 
            m.nombreMascota as mascota, hist.estado FROM historial_mascota hist 
            inner join mascotadatos m  on hist.FK_idMascota = m.idMascota 
            inner join dueno d on m.FK_idDueno = m.FK_idDueno ;");
            $stm->execute();
